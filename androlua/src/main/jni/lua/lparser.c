...
/*
** AndroLua+ 扩展：defer <语句>
** 把语句包成闭包并登记为 to-be-closed 变量，离开当前块时执行。
** Lua 5.4 原生就有 OP_TBC，这里复用它；配套的 lfunc.c 允许函数直接作为
** 可关闭值，从而省去 __close 元方法。
*/
...
static void deferstat (LexState *ls) {
  expdesc b;
  FuncState *fs = ls->fs;
  int fvar = fs->nactvar;  /* 隐藏变量的位置 */
  int vidx = new_localvar(ls, luaS_newliteral(ls->L, "(defer)"));
  getlocalvardesc(fs, vidx)->vd.kind = RDKTOCLOSE;
  adjustlocalvars(ls, 1);  /* 进入作用域，占住寄存器 */
  deferbody(ls, &b, ls->linenumber);  /* 闭包生成在该寄存器 */
  checktoclose(fs, fvar);
